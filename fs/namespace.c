...
static int susfs_mnt_start = DEFAULT_KSU_MNT_ID;
...
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	if (mnt->mnt_group_id >= susfs_mnt_group_start) {
...
static struct mount *susfs_alloc_non_unshare_ksu_vfsmnt(const char *name)
{
	...
	if (mnt) {
		res = ida_simple_get(&susfs_mnt_id_ida, susfs_mnt_start, 0, GFP_KERNEL);;
		if (res < 0) {
			goto out_free_cache;
		}
		mnt->mnt_id = res;
...
int susfs_get_non_sus_mnt_id_from_mnt(struct mount *orig_mnt) {
	...
	lock_mount_hash();
	for (; mnt && mnt->mnt_parent && mnt != mnt->mnt_parent && mnt->mnt_id >= susfs_mnt_start; mnt = mnt->mnt_parent) { }
	mnt_id = mnt->mnt_id;
	unlock_mount_hash();
	return mnt_id;
}
